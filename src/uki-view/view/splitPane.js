...
uki.view.declare('uki.view.HSplitPane', uki.view.Container, function (Base) {
  ...
  this._normalizePosition = function (val) {
    ...
    return MAX(
    this._leftMin, MIN(
    this._rect[prop] - this._rightMin - this._handleWidth, MAX(0, MIN(this._rect ? this._rect[prop] : 1000, val * 1))));
  };
  ...
  this._createHandle = function () {
    var handle;
    if (this._vertical) {
      handle = uki.theme.dom('splitPane-vertical', {
        handleWidth: this._handleWidth
      });
      handle.style.top = this._handlePosition + PX;
    } else {
      handle = uki.theme.dom('splitPane-horizontal', {
        handleWidth: this._handleWidth
      });
  ...
  this._createDom = function () {
    ...
    this._initClassName();
    for (var i = 0, paneML; i < 2; i += 1) {
      paneML = {
        view: 'Container'
      };
      paneML.anchors = i === 1 ? 'left top bottom right' : this._vertical ? 'left top right' : 'left top bottom';
  ...
  this._resizeSelf = function (newRect) {
    ...
    if (!Base._resizeSelf.call(this, newRect)) {
      return false;
    }
  ...
  this._childViewsAt = function (i, views) {
    if (views === undefined) {
      return this._panes[i].childViews();
    }
  ...
  this._rightRect = function () {
    if (this._vertical) {
      return new Rect(
      0, this._handlePosition + this._handleWidth, this._rect.width, this._rect.height - this._handleWidth - this._handlePosition);
    } else {
      return new Rect(
      this._handlePosition + this._handleWidth, 0, this._rect.width - this._handleWidth - this._handlePosition, this._rect.height);
    }
  };
