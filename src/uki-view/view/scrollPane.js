(function () {
  ...
  function initScrollWidth() {
    if (!scrollWidth) {
      uki.dom.probe(
      uki.createElement('div', 'position:absolute;left:-99em;width:100px;height:100px;overflow:scroll;', '<div style="position:absolute;left:0;width:100%;"></div>'), function (probe) {
  ...
  uki.view.declare('uki.view.ScrollPane', uki.view.Container, function (Base) {
    ...
    this.rectForChild = function () {
      return this._rectForChild;
    };
    ...
    this._recalcClientRects = function () {
      ...
      this._clientRect = new Rect(this._rect.width + (sv ? -1 : 0) * scrollWidth, this._rect.height + (sh ? -1 : 0) * scrollWidth);
      this._rectForChild = new Rect(this._rect.width + ((sv && !widthIncludesScrollBar) ? -1 : 0) * scrollWidth, this._rect.height + ((sh && !widthIncludesScrollBar) ? -1 : 0) * scrollWidth);
    };
    ...
    this._resizeChildViews = function (oldClientRect) {
      for (var i = 0, childViews = this.childViews(); i < childViews.length; i += 1) {
