...
uki.view.declare('uki.view.List', uki.view.Base, uki.view.Focusable, function (Base, Focusable) {
  ...
  this.addRow = function (position, data) {
    ...
    if (position <= this._packs[0].itemTo) {
      this._packs[0].itemTo += 1;
      this._packs[1].itemFrom += 1;
      this._packs[1].itemTo += 1;
    ...
    for (var i = selectionPosition; i < this._selectedIndexes.length; i += 1) {
      this._selectedIndexes[i] += 1;
    }
  ...
  this.redrawRow = function (position) {
    var item = this._itemAt(position);
    if (item) {
      item.innerHTML = this._render.render(this._data[position], this._rowRect(position), position);
    }
    return this;
  };
  ...
  this.selectedIndexes = function (indexes) {
    ...
    for (var i = 0; i < this._selectedIndexes.length; i += 1) {
  ...
  function range(from, to) {
    ...
    for (var idx = 0; from <= to; from += 1, idx += 1) {
      result[idx] = from;
    }
    return result;
  }
  ...
  function removeRange(array, from, to) {
    ...
    if (p > initialP) {
      array.splice(initialP, p - initialP);
    }
  }
    ...
    if (this._multiselect) {
      ...
      if (e.shiftKey && indexes.length > 0) {
        if (this.isSelected(p)) {
          ...
          Math.min(p, indexes[0]), Math.max(p, indexes[indexes.length - 1])));
        }
      } else if (e.metaKey) {
  ...
  this._restorePackSelection = function (pack) {
    ...
    if ((indexes[0] <= pack.itemFrom && indexes[indexes.length - 1] >= pack.itemFrom) || // left index
  ...
  this._layoutDom = function (rect) {
    ...
    if (this._focusTarget) {
      this._focusTarget.style.top = this._visibleRect.y + 'px';
    }
...
uki.view.declare('uki.view.ScrollableList', uki.view.ScrollPane, function (Base) {
  ...
  this._createDom = function () {
    ...
    this._list = uki({
      view: 'List',
      rect: this.rect().clone().normalize(),
      anchors: 'left top right bottom'
    })[0];
    this.appendChild(this._list);
  };

  uki.each('data rowHeight render packSize visibleRectExt throttle focusable selectedIndex selectedIndexes selectedRow selectedRows multiselect draggable textSelectable'.split(' '), function (i, name) {
