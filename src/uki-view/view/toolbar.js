...
uki.view.declare('uki.view.Toolbar', uki.view.Container, function (Base) {
  ...
  this.buttons = uki.newProp('_buttons', function (b) {
    ...
    this._totalWidth = uki.reduce(0, this._flow.childViews(), function (s, v) {
      return s + v.rect().width;
    });
  });
  ...
  this._createDom = function () {
    ...
    var rect = this.rect(),
      ...
      className: 'toolbar-flow',
      hidePartlyVisible: true
    },
      moreML = {
      view: 'Button',
      rect: moreRect,
      ...
      background: 'theme(toolbar-popup)',
      childViews: {
        view: 'VFlow',
        rect: '0 5 0 0',
        anchors: 'right top left bottom'
      }
    };
  ...
  this._showMissingButtons = function () {
    var maxWith = this._flow.rect().width,
      currentWidth = 0,
      missing = [];
    for (var i = 0, childViews = this._flow.childViews(), l = childViews.length; i < l; i += 1) {
      currentWidth += childViews[i].rect().width;
      if (currentWidth > maxWith) {
        missing.push(i);
      }
    };
    var newButtons = uki.map(missing, function (i) {
      var descr = {
        html: childViews[i].html(),
        backgroundPrefix: 'toolbar-popup-button-'
      };
