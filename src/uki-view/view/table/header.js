...
uki.view.declare('uki.view.table.Header', uki.view.Label, function (Base) {
  ...
  this._click = function (e) {
    ...
    var target = e.target;
    if (target === this.dom() || target === this._label) {
      return;
    }
    while (target.parentNode !== this._label) {
  ...
  this.redrawColumn = function (col) {
    ...
    if (this._columns[col].resizable()) {
      this._createResizers(col);
    }
  };

  this._createColumns = function () {
    var html = [];
    for (var i = 0, offset = 0, columns = this._columns, l = columns.length; i < l; i += 1) {
  ...
  this._bindResizerDrag = function (resizer, columnIndex) {
    ...
    uki.dom.bind(resizer, 'draggesture', function (e) {
      ...
      var headerOffset = uki.dom.offset(_this.dom()),
        ...
        columnOffset = 0,
        i, column = _this._columns[columnIndex];
      for (i = 0; i < columnIndex; i += 1) {
