...
(function () {
  ...
  var self = uki.background = function (bg) {
    if (typeof(bg) === 'string') {
      var match;
      /*jsl:ignore*/
      if (match = bg.match(nullRegexp)) {
        return new self.Null();
      }

      if (match = bg.match(themeRegexp)) {
        return uki.theme.background(match[1]);
      }
      if (match = bg.match(rowsRegexp)) {
        return new self.Rows(match[1].split(',')[0], match[1].split(/\s*,\s*/).slice(1));
      }
      if (match = bg.match(cssBoxRegexp)) {
        return new self.CssBox(match[1]);
      }
      /*jsl:end*/
