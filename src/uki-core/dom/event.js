...
/** @lends uki.dom */
{
  ...
  bind: function (el, types, listener) {
    ...
    var id = el[expando] = el[expando] || uki.guid++,
        handler = uki.dom.handlers[id] = uki.dom.handlers[id] ||
        function () {
  ...
  unbind: function (el, types, listener) {
    ...
    if (types) {
      ...
      types = [];
      uki.each(uki.dom.bound[id] || [], function (k, v) {
        types.push(k);
      });
    }
    for (i = 0; i < types.length; i += 1) {
      type = types[i];
      if (!id || !uki.dom.bound[id] || !uki.dom.bound[id][type]) {
        continue;
      }
      uki.dom.bound[id][type] = listener ? uki.grep(uki.dom.bound[id][type], function (h) {
        return h.huid !== huid;
      }) : [];
  ...
  handler: function (e) {
    ...
    if (!id || !handlers || !handlers[type]) {
      return;
    }

    uki.after.start();
    for (i = 0, handlers = handlers[type]; i < handlers.length; i += 1) {
