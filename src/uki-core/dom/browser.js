...
function () {
  ...
  this.canvas = function () {
    if (canvas === undefined) {
      canvas = !! uki.createElement('canvas').getContext;
    }
    return canvas;
  };
  ...
  this.cssFilter = function () {
    if (filter === undefined) {
      filter = typeof uki.createElement('div').style.filter !== 'undefined';
    }
    return filter;
  };
  ...
  this.css = function (css) {
    ...
    if (typeof css === 'string') {
      return css.replace(/(^|[^-])(box-shadow|border-radius|user-select)/g, function (value) {
        var p;
        if ((p = value.indexOf('box-shadow')) > -1) {
          return value.substr(0, p) + uki.browser.cssBoxShadow();
        }
        if ((p = value.indexOf('border-radius')) > -1) {
          return value.substr(0, p) + uki.browser.cssBorderRadius();
        }
        if ((p = value.indexOf('user-select')) > -1) {
          return value.substr(0, p) + uki.browser.cssUserSelect();
        }
      });
    }
    ...
    uki.each(['boxShadow', 'borderRadius', 'userSelect'], function (k, v) {
      if (css[v]) {
        swap(css, v, uki.camalize(uki.browser[uki.camalize('css-' + v)]()));
      }
    });
    return css;
  };
  ...
  function checkPrefixes(dashProp) {
    ...
    for (var i = 0; i < prefixes.length; i += 1) {
      if (style[uki.camalize(prefixes[i] + dashProp)] !== undefined) {
        return prefixes[i] + dashProp;
      }
    };
    return 'unsupported';
  }
  ...
  function initLinearGradient() {
    var e = uki.createElement('div', 'background-image:-moz-linear-gradient(right,red,red);' + 'background-image:linear-gradient(right,red,red);' + 'background-image:-webkit-gradient(linear,0 0,100% 0,from(red),to(red))'),
      style = e.style,
...
uki.initNativeLayout = function () {
  if (uki.supportNativeLayout === undefined) {
    uki.dom.probe(
    uki.createElement('div', 'position:absolute;width:100px;height:100px;left:-999em;', '<div style="position:absolute;left:0;right:0"></div>'), function (div) {
