...
(function () {
  ...
  uki.dom.DataTransfer = uki.newClass(new
  function () {
    this.init = function () {
      uki.extend(this, {
        dropEffect: 'none',
    ...
    this.setData = function (format, data) {
      ...
      if (uki.inArray(format, this.types) === -1) {
    ...
    this.clearData = function (format) {
      if (format) {
        ...
        this.types = uki.grep(this.types, function (x) {
          return x !== format;
        });
      } else {
        this.data = {};
  ...
  var bindW3CDrag = {
    ...
    teardown: function () {
      ...
      if (!this.__draggesturebound) {
        uki.dom.unbind(this, 'draggesture', drag);
      }
    }
  };
  ...
  function dragenter(e) {
    if (!dnd.dataTransfer || e.domEvent.__dragEntered || !retriggering) {
      return;
    }
    e = new uki.dom.Event(e);
    e.domEvent.__dragEntered = true;
    if (dnd.dragOver === this) {
      return;
    }
    dnd.dragOver = this;
    e.type = 'dragenter';
  ...
  function drag(e) {
    ...
    if (dnd.dataTransfer) { // dragging
      ...
      try {
        while (target && target !== parent && !target.getAttribute('draggable')) {
    ...
    try {
      ...
      try {
        if (doc.createEventObject) {
          ...
          type, true, true, doc.defaultView, e.detail, e.screenX, e.screenY, e.clientX, e.clientY, e.ctrlKey, e.altKey, e.shiftKey, e.metaKey, e.button, null);
