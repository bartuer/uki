...
uki.view.declare('uki.view.Container', uki.view.Base, function (Base) {
  ...
  function maxProp(c, prop) {
    var val = 0,
      i, l;
    for (i = c._childViews.length - 1; i >= 0; i -= 1) {
      if (!c._childViews[i].visible()) {
        continue;
      }
  ...
  this.removeChild = function (child) {
    ...
    var index = child._viewIndex,
      i, l;
    for (i = index + 1, l = this._childViews.length; i < l; i += 1) {
      this._childViews[i]._viewIndex--;
    };
    this._childViews = uki.grep(this._childViews, function (elem) {
      return elem !== child;
    });
  ...
  this.insertBefore = function (child, beforeChild) {
    ...
    for (i = beforeChild._viewIndex, l = this._childViews.length; i < l; i += 1) {
      this._childViews[i]._viewIndex++;
    };
