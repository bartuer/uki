 ...
 * @interface
 */
uki.view.Focusable = new
function () {
  /** @lends uki.view.Focusable.prototype */
  ...
  this.disabled = uki.newProp('_disabled', function (d) {
    ...
    this._updateFocusable();
    if (changed && this._updateBg) {
      this._updateBg();
    }
  });

  this._updateFocusable = function () {
    if (this._preCreatedFocusTarget || !this._focusTarget) {
      return;
    }
  ...
  this._initFocusable = function (preCreatedFocusTarget) {
    if ((!preCreatedFocusTarget && !this._focusable) || this._focusTarget) {
      return;
    }
  ...
  this._bindToDom = function (name) {
    if (!this._focusTarget || 'keyup keydown keypress focus blur'.indexOf(name) === -1) {
      return false;
    }
