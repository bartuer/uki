...
uki.view.declare('uki.view.Base', uki.view.Observable, uki.view.Styleable, function (Observable, Styleable) {
  ...
  this.background = function (val) {
    if (val === undefined && !this._background && this.defaultBackground) {
      this._background = this.defaultBackground();
    }
    if (val === undefined) {
  ...
  this.anchors = uki.newProp('_anchors', function (anchors) {
    if (anchors.indexOf) {
      ...
      if (anchors.indexOf('width') > -1 || (tmp & ANCHOR_LEFT && tmp & ANCHOR_RIGHT)) {
        tmp |= ANCHOR_WIDTH;
      }
      if (anchors.indexOf('height') > -1 || (tmp & ANCHOR_BOTTOM && tmp & ANCHOR_TOP)) {
        tmp |= ANCHOR_HEIGHT;
      }
      anchors = tmp;
    }
  ...
  this.layoutIfNeeded = function () {
    if (this._needsLayout && this.visible()) {
      this.layout();
    }
  };
  ...
  this.parentResized = function (oldSize, newSize) {
    var newRect = this._parentRect.clone(),
      dX = (newSize.width - oldSize.width) / ((this._anchors & ANCHOR_LEFT ^ ANCHOR_LEFT ? 1 : 0) + // flexible left
    (this._anchors & ANCHOR_WIDTH ? 1 : 0) + (this._anchors & ANCHOR_RIGHT ^ ANCHOR_RIGHT ? 1 : 0)),
      // flexible right
    dY = (newSize.height - oldSize.height) / ((this._anchors & ANCHOR_TOP ^ ANCHOR_TOP ? 1 : 0) + // flexible top
    (this._anchors & ANCHOR_HEIGHT ? 1 : 0) + (this._anchors & ANCHOR_BOTTOM ^ ANCHOR_BOTTOM ? 1 : 0)); // flexible right
    if (this._anchors & ANCHOR_LEFT ^ ANCHOR_LEFT) {
      newRect.x += dX;
    }
    if (this._anchors & ANCHOR_WIDTH) {
      newRect.width += dX;
    }

    if (this._anchors & ANCHOR_TOP ^ ANCHOR_TOP) {
      newRect.y += dY;
    }
    if (this._anchors & ANCHOR_HEIGHT) {
      newRect.height += dY;
    }
    this.rect(newRect);
  };
  ...
  function decodeAutosize(autosizeStr) {
    ...
    var autosize = 0;
    if (autosizeStr.indexOf('width') > -1) {
      autosize = autosize | ANCHOR_WIDTH;
    }
    if (autosizeStr.indexOf('height') > -1) {
      autosize = autosize | ANCHOR_HEIGHT;
    }
    return autosize;
  }
  ...
  this._layoutDom = function (rect) {
    var l = {},
      s = uki.supportNativeLayout,
      relativeRect = this.parent().rectForChild(this);
  ...
  this._bindToDom = function (name) {
    if ('resize layout'.indexOf(name) > -1) {
      return true;
    }
