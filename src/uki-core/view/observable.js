 ...
 */
uki.view.Observable =
/** @lends uki.view.Observable.prototype */
{
  // dom: function() {
  ...
  unbind: function (name, callback) {
    ...
    if (name) {
      ...
      names = [];
      uki.each(this._observers, function (k, v) {
        names.push(k);
      });
    }
  ...
  _unbindFromDom: function (name) {
    if (!this._domHander || !this._eventTargets[name]) {
      return;
    }
  ...
  _observersFor: function (name, skipCreate) {
    if (skipCreate && (!this._observers || !this._observers[name])) {
      return [];
    }
    if (!this._observers) {
      this._observers = {};
