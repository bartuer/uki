...
uki.background.CssBox = uki.newClass(new function () {
  ...
  function getInsets(options) {
    if (!cache[options]) {
      uki.dom.probe(
      uki.createElement('div', options + ';position:absolute;overflow:hidden;left:-999em;width:10px;height:10px;'), function (c) {
        cache[options] = new Inset(
        c.offsetHeight - 10, c.offsetWidth - 10);
      });
    }
  ...
  this.init = function (options, ext) {
    ...
    this._container = uki.createElement('div', 'position:absolute;overflow:hidden;z-index:' + (ext.zIndex || '-1') + ';' + 'left:' + inset.left + 'px;top:' + inset.top + 'px;right:' + inset.right + 'px;bottom:' + inset.bottom + 'px;' + uki.browser.css(options), ext.innerHTML);
