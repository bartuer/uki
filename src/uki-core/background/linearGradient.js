 ...
 * @class
 */
uki.background.LinearGradient = uki.newClass(uki.background.CssBox, new
function () {
  ...
  function getGradientURL(startColor, endColor, horizontal, stops) {
    var key = startColor + endColor + (horizontal ? 1 : 0) + uki.map(stops, function (stop) {
      return stop.pos + '-' + stop.color;
    });
    if (!urlCache[key]) {
  ...
  this._createContainer = function () {
    ...
    if (uki.browser.canvas() && (url = getGradientURL(startColor, endColor, horizontal, stops))) {
      ...
      var child;
      for (; i < stops.length - 1; i += 1) {
        child = uki.createElement('div', 'position:absolute;z-index:-1' + ';left:' + (horizontal ? stops[i].pos * 100 - (i && 1) + '%' : '0') + ';top:' + (horizontal ? '0' : stops[i].pos * 100 - (i && 1) + '%') + ';width:' + (horizontal ? (stops[i + 1].pos - stops[i].pos) * 100 + 1 + '%' : '100%') + ';height:' + (horizontal ? '100%' : (stops[i + 1].pos - stops[i].pos) * 100 + 1 + '%') + ';' + mosFilter(horizontal, stops[i].color, stops[i + 1].color));
