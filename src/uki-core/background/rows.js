...
uki.background.Rows = uki.newClass(new function () {
  ...
  this.init = function (height, colors) {
    ...
    if (this._colors.length === 1) {
      this._colors = this._colors.concat(['#FFF']);
    }
  };
  ...
  this.attachTo = function (comp) {
    ...
    this._comp = comp;
    if (!this._container) {
      this._container = uki.createElement('div', 'position:absolute;left:0;top:0;width:100%;z-index:-1');
      this._container.className = 'uki-background-Rows';
    }
    this._layoutHandler = this._layoutHandler || uki.proxy(function (e) {
      this.layout(e.rect, e.visibleRect);
    }, this);
  ...
  function getPackHTML(height, colors) {
    ...
    if (!cache[key]) {
      for (i = 0; i < l; i += 1) {
        rows[i] = ['<div style="height:', height, 'px;width:100%;overflow:hidden;', (colors[i] ? 'background:' + colors[i] : ''), '"></div>'].join('');
      }
