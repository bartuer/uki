...
(function () {
  ...
  var utils = {
    ...
    map: function (elems, callback, context) {
      var ret = [],
        mapper = uki.isFunction(callback) ? callback : function (e) {
        return uki.attr(e, callback);
      };
    ...
    methods) {
      ...
      i, startFrom = 0,
        tmp, baseClasses = [],
        base, name, copy, $arguments = arguments,
        length;
      ...
      if ((length = $arguments.length) > 1) {
        ...
        if (base.prototype) { // real inheritance
          ...
          for (name in base) {
            copy = base[name];
            if (!base.hasOwnProperty(name) || copy === undefined || name === 'prototype') {
              continue;
            }
            klass[name] = copy;
          }
    ...
    delegateProp: function (proto, name, target) {
      ...
      proto[name] = function (value) {
        if (value === undefined) {
          if (this[target]) {
            return uki.attr(this[target], name, value);
          }
