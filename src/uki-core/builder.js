...
(function () {
  ...
  function createSingle(mlRow) {
    ...
    if (uki.isFunction(c)) {
      ...
      for (var i = 0, ns = uki.viewNamespaces, ns$length = ns.length; i < ns$length; i += 1) {
        var parts = (ns[i] + c).split('.'),
          obj = root;

        for (var j = 0, parts$length = parts.length; obj && j < parts$length; j += 1) {
          obj = obj[parts[j]];
        }
        if (obj) {
  ...
  function copyAttrs(comp, mlRow) {
    uki.each(mlRow, function (name, value) {
      if (name === 'view' || name === 'type' || name === 'rect') {
        return;
      }
