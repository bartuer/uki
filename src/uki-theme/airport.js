(function () {
  ...
  uki.theme.airport = uki.extend({}, uki.theme.Base, {
    ...
    backgrounds: {
      ...
      'button-focus': function () {
        if (uki.browser.cssBoxShadow() === 'unsupported') {
          return new uki.background.CssBox('background:#7594D2;' + (uki.browser.cssFilter() && uki.image.needAlphaFix ? 'filter:Alpha(opacity=70);' : 'opacity:0.7;'), {
            inset: '-2 -2',
            zIndex: -2
          });
        }
      ...
      'button-disabled': function () {
        ...
        uki.theme.background('button-normal'), new uki.background.Css({
          color: '#999'
        }));
      },
      ...
      'checkbox-focus': function () {
        if (uki.image.needAlphaFix) {
          return new uki.background.CssBox('', {
            innerHTML: '<div style="position:absolute;left:50%;top:50%;width:19px;height:19px;overflow:hidden;' + 'margin:-10px 0 0 -10px;background:#7594D2;filter:Alpha(opacity=70);"></div>',
            zIndex: -2
          });
        }
        return new uki.background.CssBox('', {
          innerHTML: '<div style="position:absolute;left:50%;top:50%;width:24px;height:24px;overflow:hidden;' + 'margin:-12px 0 0 -12px; background: url(' + uki.theme.imageSrc('checkbox-focus') + ') 0 0"></div>',
          zIndex: -2
        });
      },
      ...
      'radio-focus': function () {
        if (uki.image.needAlphaFix) {
          return uki.theme.airport.background('checkbox-focus');
        }
        var src = uki.theme.imageSrc('radio-focus');
        return new uki.background.CssBox('', {
          innerHTML: '<div style="position:absolute;left:50%;top:50%;width:24px;height:24px;overflow:hidden;' + 'margin:-12px 0 0 -12px; background: url(' + src + ') 0 0"></div>',
          zIndex: -2
        });
      },
      ...
      // text field
      'input': function () {
        return new uki.background.CssBox('background:white;border: 1px solid #999;border-top-color:#777;box-shadow:0 1px 0 rgba(255, 255, 255, 0.4), inset 0 1px 2px rgba(0,0,0,0.2);');
      },
      ...
      'slider-handle-focus': function () {
        ...
        return new uki.background.CssBox('box-shadow: 0 0 6px #0244D4;', {
          zIndex: 2,
          inset: '1'
        });
      },
    ...
    templates: {
      'table-header-cell': function () {
        return new uki.theme.Template('<div style="position:relative;border-right:1px solid #CCC;' + '${style}" class="${className}">${data}</div>');
      },

      'table-cell': function () {
        return new uki.theme.Template('<div style="position:relative;border-right:1px solid #CCC;height:100%;' + '${style}" class="${className}">${data}</div>');
      },

      'table-header-cell-asc': function () {
        return new uki.theme.Template('<div style="position:relative;border-right:1px solid #CCC;background: rgba(0,0,128,0.1);' + '${style}" class="${className}"><div style="padding-right:7px">${data}</div><span style="position:absolute;right:0;top:50%;margin-top:-7px;">&darr;</span></div>');
      },

      'table-header-cell-desc': function () {
        return new uki.theme.Template('<div style="position:relative;border-right:1px solid #CCC;background: rgba(0,0,128,0.1);' + '${style}" class="${className}"><div style="padding-right:7px">${data}</div><span style="position:absolute;right:0;top:50%;margin-top:-7px;">&uarr;</span></div>');
      }
    },
    ...
    doms: {
      'resizer': function (height) {
        var template = new uki.theme.Template('position:absolute;width:5px;top:0;right:-3px;height:${height}px;cursor:col-resize;cursor:ew-resize;z-index:101;background:url(' + uki.theme.imageSrc('x') + ')'),
          node = uki.createElement('div', template.render({
          height: height
        }));

        if (!node.style.cursor || window.opera) {
      ...
      'splitPane-vertical': function (params) {
        var commonVerticalStyle = 'cursor:row-resize;cursor:ns-resize;z-index:200;overflow:hidden;',
          handle = params.handleWidth === 1 ? uki.createElement('div', defaultCss + 'width:100%;height:5px;margin-top:-2px;' + commonVerticalStyle + 'background: url(' + uki.theme.imageSrc('x') + ')', '<div style="' + defaultCss + 'background:#999;width:100%;height:1px;left:0px;top:2px;overflow:hidden;' + '"></div>') : uki.createElement('div', defaultCss + 'width:100%;height:' + (params.handleWidth - 2) + 'px;' + 'border: 1px solid #CCC;border-width: 1px 0;' + commonVerticalStyle + 'background: url(' + uki.theme.imageSrc('splitPane-vertical') + ') 50% 50% no-repeat;');
        if (!handle.style.cursor || window.opera) {
      ...
      'splitPane-horizontal': function (params) {
        var commonHorizontalStyle = 'cursor:col-resize;cursor:ew-resize;z-index:200;overflow:hidden;',
          handle = params.handleWidth === 1 ? uki.createElement('div', defaultCss + 'height:100%;width:5px;margin-left:-2px;' + commonHorizontalStyle + 'background: url(' + uki.theme.imageSrc('x') + ')', '<div style="' + defaultCss + 'background:#999;height:100%;width:1px;top:0px;left:2px;overflow:hidden;' + '"></div>') : uki.createElement('div', defaultCss + 'height:100%;width:' + (params.handleWidth - 2) + 'px;' + 'border: 1px solid #CCC;border-width: 0 1px;' + commonHorizontalStyle + 'background: url(' + uki.theme.imageSrc('splitPane-horizontal') + ') 50% 50% no-repeat;');
        if (!handle.style.cursor || window.opera) {
  ...
  function checkboxBg(offset) {
    ...
    return new uki.background.CssBox('', {
      innerHTML: '<div style="position:absolute;left:50%;top:50%;width:18px;height:18px;overflow:hidden;' + 'margin:-9px 0 0 -9px; background: url(' + src + ') 0 -' + offset + 'px"></div>'
    });
  }
