...
uki.view.declare('uki.more.view.NativeInput', uki.view.Base, uki.view.Focusable, function (Base, Focusable) {
  ...
  this._layoutDom = function (rect) {
    ...
    this._lastInputLayout = uki.dom.layout(this._input.style, {
      width: rect.width
    }, this._lastInputLayout);
  ...
  function getInputOffset(input) {
    ...
    return (uki.dom.probe(clone2, function (clone) {
      return clone.offsetWidth;
    }) - uki.dom.probe(clone1, function (clone) {
      return clone.offsetWidth;
    })) / 2;
  }
  ...
  this._updatePlaceholderVis = function () {
    if (this._placeholderDom) {
      this._placeholderDom.style.display = this.value() ? 'none' : 'block';
    }
  };
  ...
  this._focus = function (e) {
    if (this._placeholderDom) {
      this._placeholderDom.style.display = 'none';
    }
