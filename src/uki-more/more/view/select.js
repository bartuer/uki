...
uki.view.declare('uki.more.view.Select', uki.view.Checkbox, function (Base) {
  ...
  this._updateWidth = function () {
    ...
    var source = this._list.dom().firstChild.firstChild.firstChild,
      /// omg!
    ...
    this._popup.rect().x, this._popup.rect().y, Math.max(this._widthCached, this.rect().width), Math.min(this._maxPopupHeight, this._options.length * 22))).layout();
  };

  this.options = uki.newProp('_options', function (o) {
    this._options = o;
    this._list.data(uki.map(o, 'text')).selectedIndex(0);

    if (this._selectFirst && (o.length > 0)) {
      this.text(o[0].text);
    }
    this._longestText = '';
    uki.each(o, function (i, row) {
      if (row.text.length > this._longestText.length) {
        this._longestText = row.text;
      }
    }, this);
