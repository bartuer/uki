  ...
  var theme = uki.extend({}, uki.theme.Base, {
    backgrounds: {
      ...
      'select-normal': function () {
        ...
        selectHandle('select-handle-normal'), uki.theme.background('button-normal'));
      },
      'select-hover': function () {
        return new uki.background.Multi(
        selectHandle('select-handle-normal'), uki.theme.background('button-hover'));
      },
      'select-checked-normal': function () {
        return new uki.background.Multi(
        selectHandle('select-handle-normal'), uki.theme.background('button-down'));
      },
      'select-disabled': function () {
        return new uki.background.Multi(
        selectHandle('select-handle-normal', 'opacity:0.4;'), uki.theme.background('button-disabled'));
      },
